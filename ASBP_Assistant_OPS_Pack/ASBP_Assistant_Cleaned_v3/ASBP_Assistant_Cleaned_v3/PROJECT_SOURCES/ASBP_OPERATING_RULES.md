...
Its purpose is to keep the project strict without making the assistant fragile. The assistant must separate public-facing documentation work, roadmap governance, implementation truth, tracker state, and repository workflow hygiene.
...
Expected repository-side authority files:
...
7. `ASBP_BRANCH_PR_MERGE_STRATEGY.md` from Project Sources for repository workflow decisions
8. `STARTER_PROMPT.md` from Project Sources when starting or restarting a session
...
### Branch / PR / merge strategy

`ASBP_BRANCH_PR_MERGE_STRATEGY.md` governs repository workflow hygiene.

It decides how work should be grouped into branches and pull requests.

It does not change roadmap order, checkpoint authority, architecture rules, validation policy, or implementation truth.
...
7. read `ASBP_BRANCH_PR_MERGE_STRATEGY.md` from Project Sources when branch/PR/merge guidance is needed
8. do not read old learning documents
9. do not read README as current-state authority
10. confirm only:
   - current phase
   - current milestone
   - current approved slice family
...
### Lag types that do not stop execution

Do not stop execution for these alone:

- terminology lag
- tracker formatting lag
- repo naming lag
- public-surface lag
- branch/PR workflow-format lag

Correct them when asked, but do not create addenda for them.
...
- Do not invent ad hoc checkpoints.
- Do not skip, reorder, or silently merge roadmap checkpoints.
- Do not confuse PR bundling with checkpoint execution: PR bundles may group related completed work for review, but they do not change roadmap order.
...
Public-surface maintenance may receive a minimal tracker note only when the workflow requires it.

---

## Pull request workflow guidance

Use `ASBP_BRANCH_PR_MERGE_STRATEGY.md` for detailed workflow decisions.
...
Do not create one PR for every tiny slice automatically.

Do not create one huge milestone PR that hides unrelated implementation work.
...
- grouped capability implementation
- milestone closure
- public-surface packages intended to be reviewed before merging
...
- PR workflow is hygiene, not bureaucracy.
